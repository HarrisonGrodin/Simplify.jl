...
solve(σ::Substitution, (f, g)::Tuple{Fn,Fn}, ms...) =
    sig(f) == sig(g) ? solve(σ, zip(f, g)..., ms...) : nothing
...
matches(σ::Substitution, (f, g)::Tuple{Fn,Fn}, ms...) =
    sig(f) == sig(g) ? matches(σ, zip(f, g)..., ms...) : nothing
